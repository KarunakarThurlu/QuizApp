...
const ExamsTable = () => {
  ...
  const handleExamReviewClick = (row) => {
    ...
    if (
      ...
      TestQuestions.length === TestAnswers.length
    ) {
      let AnsMap = new Map();
      TestAnswers.forEach((ans) => {
        AnsMap.set(Object.keys(ans)[0], Object.values(ans)[0]);
      });
      TestQuestions.forEach((que) => {
        que["YourAnswer"] = AnsMap.get(que._id);
      });
    }
  ...
  const columns = [
    {
      ...
      title: "Id",
      render: (params) => (
        <ToolTip key={params._id} toolTipData={params._id} length={8} />
      ),
    },
    {
    ...
    { field: "TestScore", title: "TestScore" },
    {
      ...
      title: "Review",
      render: (row) => (
        <Button variant="contained" onClick={() => handleExamReviewClick(row)}>
  ...
  return (
    <div className="Exams-Table">
      ...
      <ReviewExam
        open={showReviewExam}
        onClose={() => setShowReviewExam(false)}
        data={testQuestions}
        score={testScore}
      />
      <DeletePopUpModel
        open={openDeleteModel}
        onClickYes={handleConformDelete}
        message={CommonConstants.Delete_Exam_Warning}
