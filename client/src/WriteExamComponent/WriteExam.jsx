...
function WriteExam(props) {
  ...
  const [warningMessage, setWarningMessage] = useState(
    CommonConstants.Submit_Exam_Warning
  );
  ...
  return (
    <div className="write-exam-container">
      ...
      {openSubmitWarningModel && (
        <WarningPopUpModel
          open={openSubmitWarningModel}
          message={warningMessage}
          onClickYes={handleExamSubmit}
          handleClose={() => setOpenSubmitWarningModel(false)}
        />
      )}
      {openExamResultsModel && (
        <ExamResults
          open={openExamResultsModel}
          handleClose={() => setOpenExamResultsModel(false)}
          testScore={testScore}
        />
      )}
      {loader && <Spinner open={loader} />}
      {startExam === true ? (
        <Grid container className="writeexam" spacing={0}>
          <Grid item xs={8}>
            <FormControl className="Question-name-form-controll">
              <FormLabel>
                {questionInfo.id}.{questionInfo.name}
              </FormLabel>
              <RadioGroup
                value={value}
              ...
              >
                <FormControlLabel
                  ...
                  control={
                    <Radio
                      checked={questionInfo.answer === "A" ? true : false}
                    />
                  }
                  label={questionInfo.optionA}
                />
                <FormControlLabel
                  value="B"
                  onClick={(e) => saveAnswer(e)}
                  control={
                    <Radio
                      checked={questionInfo.answer === "B" ? true : false}
                    />
                  }
                  label={questionInfo.optionB}
                />
                <FormControlLabel
                  value="C"
                  onClick={(e) => saveAnswer(e)}
                  control={
                    <Radio
                      checked={questionInfo.answer === "C" ? true : false}
                    />
                  }
                  label={questionInfo.optionC}
                />
                <FormControlLabel
                  value="D"
                  onClick={(e) => saveAnswer(e)}
                  control={
                    <Radio
                      checked={questionInfo.answer === "D" ? true : false}
                    />
                  }
                  label={questionInfo.optionD}
                />
              </RadioGroup>
            </FormControl>
            <div className="next-buttons">
              <Button
                variant="contained"
                color="primary"
                onClick={(e) => handleNextButtons("PreviousQuestion")}
              >
              ...
              <Button
                variant="contained"
                color="primary"
                onClick={(e) => handleNextButtons("NextQuestion")}
              >
                <SkipNextRoundedIcon style={{ color: "white" }} />
              </Button>
            </div>
          </Grid>
          <Grid item xs={3}>
            ...
            >
              {questions.map((value, i) => (
                <Grid
                  ...
                  item
                  style={{
                    backgroundColor: value.answer !== "" ? "green" : "yellow",
                  }}
                  onClick={(e) => {
                    setCurrentQuestion(value.id);
              ...
              <div className="submit-button">
                <Button variant="contained" onClick={showWarning}>
                  Submit
                </Button>
              </div>
            </Grid>
        ...
        <Grid container spacing={0} className="start-exam">
          <Grid item xs={8}>
            <Typography
              ...
              align="center"
            >
              Exam Contains 20 Questions and Time also 20 minutes. Each Question
              has exactly one minute. Before starting the exam you need to
              select Topic & Questions Range then you can click on start.
            </Typography>
            <br />
            <br />
            <div className="start-exam-form-control">
              ...
              <FormControl
                variant="outlined"
                className="start-exam-form-control"
              >
