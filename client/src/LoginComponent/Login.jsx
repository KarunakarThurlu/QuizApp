...
function Login(props) {
  ...
  const validateForm = () => {
    ...
    if (data.email.length > 0) {
      ...
      errorsObj.email = HelperUtils.validateEmail(data.email)
        ? ""
        : "Invalid Email";
    }
          ...
          if (response.data.statusCode === 200) {
            ...
            if (response.data.data.roles.length === 1) {
              localStorage.setItem(
                "role",
                response.data.data.roles[0].role_name
              );
            } else if (response.data.data.roles.length === 2) {
              if (
                response.data.data.roles[0].role_name === "ADMIN" ||
                response.data.data.roles[1].role_name === "ADMIN"
              )
            ...
            Notifier.notify(
              response.data.message,
              Notifier.notificationType.ERROR
            );
          }
        })
        .catch((error) => {
  ...
  return (
    <div className="login">
      ...
      <div className="form-button">
        <Button
          onClick={onSubmit}
          className="MuiButton-containedPrimary-login"
          variant="contained"
          color="primary"
        >
          Login
      ...
      <div className="questions">
        <Link to="/signup">
          <h4 className="questions-text">New User?</h4>
        </Link>
        <Link to="/signin">
          <h4 className="questions-text">Alredy have account?</h4>
        </Link>
